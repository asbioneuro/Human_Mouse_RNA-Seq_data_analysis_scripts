...
mouse_countData <- read.csv('cul3_cerebellum_ko_wt_counts.csv', header = TRUE, sep = ",")
head(mouse_countData)
tail(mouse_countData,10)

mouse_metaData <- read.csv('cul3_cerebellum_ko_wt_metadata.csv', header = TRUE, sep = ",")
mouse_metaData


dds <- DESeqDataSetFromMatrix(countData=mouse_countData, 
                              colData=mouse_metaData, 
                              design=~dex, tidy=TRUE)
dds$dex
dds$dex <- relevel(dds$dex, ref = "wildtype")
...
write.csv(resdata, file ="results-with-normalized1.csv")
...
select(org.Mm.eg.db, keys=resdata$ENSEMBL,
       keytype = "ENSEMBL",columns=c("SYMBOL","GENENAME")
)

anno <- AnnotationDbi::select(org.Mm.eg.db,keys=resdata$ENSEMBL,
...
write.csv(results_annotated, file="cul3_cerebellum_ko_wt_deseq2_normalized_annotated.csv")
...
data <- read.csv("thap12_counts_dendogram.csv", header=TRUE, sep = ",")
...
library(pheatmap)
library(tidyverse)
countfile<- read.csv("heatmap_thap12.csv", header = TRUE, sep = ",")
head(countfile)
